...
from robot_baseline.robot.resources.walking.wj_func import WJFunc
...
class WFunc:
    ...
    def _update_new_param(self, param_pso):
        for k,v in param_pso.items():
            self.parameters[k] = -v

    def _update_new_offset(self, ang_offsets_pso):
    ...
    def apply_velocity(self, angles, velocity, phase, x):
        ...
        if phase:
            ...
            angles["l_sho_pitch"] -= d-0.2
            angles["r_sho_pitch"] -= d-0.2
        else:
            angles["l_hip_pitch"] -= d
            angles["l_ank_pitch"] -= d
            angles["r_hip_pitch"] -= d
